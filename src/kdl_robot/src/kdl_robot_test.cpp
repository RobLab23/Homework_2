...
//HomeWork 3.a-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
KDLPlanner chooseTrajectory(int trajFlag, Eigen::Vector3d init_position, Eigen::Vector3d end_position, double traj_duration, double acc_duration, double t, double init_time_slot, double traj_radius) 
{   
    switch(trajFlag) {
        case 1:
...
int main(int argc, char **argv)
{
    ...
    std::cout << "Choose desired trajectory:" << std::endl
                << "1. Linear with trapezoidal profile\n" << "2. Linear with cubic profile\n" << 
                "3. Circular with trapezoidal profile\n" << "4. Circular with cubic profile\n";
    ...
    // Plan trajectory    
    double traj_duration = 1.5, acc_duration = 0.5, t = 0.0, init_time_slot = 1.0, traj_radius = 0.1; //HomeWork 3.a : radius

    //HomeWork 3.a
    KDLPlanner planner = chooseTrajectory(trajFlag, init_position, end_position, traj_duration, acc_duration, t, init_time_slot, traj_radius);

    // Retrieve the first trajectory point
    trajectory_point p = planner.compute_trajectory(t);

    // Gains
    double Kp = 58, Kd = 10; //HomeWork 3.b
    ...
    while ((ros::Time::now()-begin).toSec() < 2*traj_duration + init_time_slot)
    {
        if (robot_state_available)
        {
            ...
            //tau = controller_.idCntr(qd, dqd, ddqd, Kp, Kd, error); //HomeWork 4.c : error
